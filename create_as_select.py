...
def create_as_select(table_name, select_fields):
	...
	assert select_fields[0] == None
	original_table = Table(select_fields[2])
	# print(original_table.get_final_field(select_fields[1]))
	final_fields = []
	fields, nicknames, needed_fields = original_table.get_final_field(select_fields[1])
	for field in fields:
		final_fields.append((field, original_table.name2type[nicknames[field]]))
