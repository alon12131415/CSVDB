	...
	def real_cmp(line1, line2):
		...
		for x1,x2,i in zip(l1, l2, ind):
			if types[i[0]] == "varchar":
				...
				f1, f2 = int(x1) if x1 not in ["NULL", NULL()] else NULL(), int(x2) if x2 not in ["NULL", NULL()] else NULL()
			elif types[i[0]] == "float":
				f1, f2 = float(x1) if x1 not in ["NULL", NULL()] else NULL(), float(x2) if x2 not in ["NULL", NULL()] else NULL()
			else:
