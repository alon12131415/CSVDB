...
class Parser():
	...
	def create_check_existing(self):
		...
		self.next_tokens()
		if self._tok == SqlTokenKind.KEYWORD and self._val == 'if':	#determine if whe need to check for the existence of the table
	...
	def create_get_fields(self):
		...
		fields = []
		while True:
			if self._tok != SqlTokenKind.IDENTIFIER: raise SyntaxError
			field = self._val
			self.next_tokens()
			if self._tok != SqlTokenKind.KEYWORD: raise SyntaxError
			fields.append((field, self._val))
			if (not self.expect(SqlTokenKind.OPERATOR, ",")):
				if self._tok == SqlTokenKind.OPERATOR and self._val == ")": break
				else:
	...
	def select_get_fields(self):
		...
		while True:
			...
			if self._val == "(":
				...
				self.expect(SqlTokenKind.OPERATOR, ")") 		#aggregators are TUPLE, ass = TrueS
				self.next_tokens()
	...
	def select_order(self):
		...
		if self._val == "order":
			...
			while True:
				...
				f = self._val
				self.next_tokens()
				if self._val in ["asc", "desc"] and self._tok == SqlTokenKind.KEYWORD:
					order_way = self._val
					self.next_tokens()
