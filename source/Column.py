...
columnLib.Column_new.argstypes = [ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int]
columnLib.Column_new.restype = ctypes.c_void_p

columnLib.Column_setFP.argstypes = [ctypes.c_void_p, ctypes.c_int]
...
class Column:
	...
	def __init__(self, table_name, field_name, field_type, file_sizes, file_num):
		self.obj = ctypes.c_void_p(columnLib.Column_new(bytes(os.path.join(table_name, field_name), encoding = "ascii"),
	...
	def setFP(self, index):
		columnLib.Column_setFP(self.obj, index)
	...
	def getRow(self, where):  # return: finished, valid(where), val
		...
		if not self.where_set_up:
			if where != {} and where["field"] == self.field_name:
				...
				if(self.field_type == "varchar"):	columnLib.Column_setWhereConst(self.obj, ctypes.c_void_p(columnLib.CreateVarcharWhereConst(bytes(where["const"], encoding = "ascii"))))
